...
while dev_max > tol & iter < iter_max
    ...
    for t = 1:time_max
        ...
        for i = 1:age_max
            ...
            ae(t,i)=wealth_E(t,i); % enterpreneurial capital owned by an entrepreneur at time t with age i
            AE(t,i)=e_weight(i)*ae(t,i); % total capital owned by all enterpreneurs at time with age i
        ...
        for i = age_T:age_max
            ...
            if rho_t(t) >= r/(1-ice_t(t)) % borrowing is profitable
                ...
                ke(t,i)=wealth_E(t,i)+loan(t,i); % enterpreneurial capital owned by an entrepreneur at time t with age i
                
            else % borrowing is not profitable
                
                loan(t,i)=0;
                ke(t,i)=wealth_E(t,i); % enterpreneurial capital owned by an entrepreneur at time t with age i
            
            end
            
            ne(t,i)=ke(t,i)*((1-alp)*(1-psi)*ksi^(1-alp)/w_t(t))^(1/alp); %  labor employed by an entrepreneur at time with age i
            KE(t,i)=e_weight(i)*ke(t,i); % total capital owned by all enterpreneurs at time with age i
            NE(t,i)=e_weight(i)*ne(t,i); % total labor employed by all enterpreneurs at time with age i
...
%     end
    ...
    dev_w_max=max(dev_w);
    dev_rho_max=max(dev_rho);
    dev_m_max=max(dev_m);
    dev_max=max([dev_w_max dev_rho_max dev_m_max])
...
save data_result m_t w_t rho_t
